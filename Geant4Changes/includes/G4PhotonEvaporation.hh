...
public:
  ...
  inline std::map<G4double,std::pair<G4int,G4int>> GetTransLevel() const;
...
private:
  ...
  G4int    fCode;
  G4int    vShellNumber;
  std::map<G4double,std::pair<G4int,G4int>>	fTransLevel;
  G4int    fStartLevel;
  G4int    fEndLevel;
  size_t   fIndex;
...
inline std::map<G4double,std::pair<G4int,G4int>> G4PhotonEvaporation::GetTransLevel() const
{ 
  return fTransLevel;
}
#endif
