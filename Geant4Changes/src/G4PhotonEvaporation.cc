...
G4PhotonEvaporation::G4PhotonEvaporation(G4GammaTransition* p)
  : fLevelManager(nullptr), fTransition(p), fPolarization(nullptr),
    fVerbose(0), fPoints(0), vShellNumber(-1), fStartLevel(-1), fEndLevel(-1), fIndex(0), 
					 ...
					 G4Fragment* nucleus)
{
  ...
  do {
    ...
    if(gamma) { 
      ...
      G4double gamma_energy= (gamma->GetMomentum()).t()/CLHEP::MeV;
      std::pair<G4int,G4int> levels=std::pair<G4int,G4int>(fStartLevel,fEndLevel);
      if(levels.first !=0 ) fTransLevel[gamma_energy]=levels;    
      if(fVerbose > 0) {
