...
// PRESETTINGS
	...
	TTreeReader myReader("Info", myFile);

	// Set variables to access the data in the tree
	//Primaries
	TTreeReaderValue<int> vPrimaries(myReader, "Primaries");
	...
	//The branch "ParticleTime" contains a vector<TVector3>; acess them as vTime
	TTreeReaderValue<vector><TVector3>> vTime(myReader, "ParticleTime");

	//cross-section
	G4double crossSection = vTime/()

	// Create histograms for the values we read

	// A histogram for the energies ranging from 0 MeV to 20 MeV with 200 bins
	TH1F* hEnergy = new TH1F("hEnergy", ";E in MeV;#entries", 100, 0, 20);
